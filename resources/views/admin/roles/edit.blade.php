                ...
                {{-- <h3 class="text-center fw-bold text-primary mb-4">Create New Role</h3> --}}
                 ...
                 <form method="POST" action="{{ route('admin.roles.update',$role->id) }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="ml-4 form-control rounded-3 @error('name') is-invalid @enderror" placeholder="Enter the name" value="{{ old('name', $role->name ?? '') }}">
                        @error('name')
                    ...
                    <div class="mt-2">
                       ...
                       <label> <input class="ml-4" type="checkbox" name="permissions[{{$permission->name}}]" value="{{$permission->name}}"
                        @if($role->hasPermissionTo($permission->name)) checked @endif>
                    {{$permission->name}} <br/></label>
                        @endforeach
                    </div>
